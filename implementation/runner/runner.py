...
def start_script():
    cmd = ['bash', '-c', '/home/shervin/pylot/pylot_start.sh']
    run_command_without_printing(cmd)

def stop_script():
    cmd = ['bash', '-c', '/home/shervin/pylot/pylot_cleanup.sh']
    run_command(cmd)

def start_ssh():
    cmd = ['nvidia-docker', 'exec', '-i', '-t', 'pylot', 'sudo', 'service', 'ssh', 'start']
    run_command(cmd)

def start_record(addr):
    process = subprocess.Popen(['recordmydesktop', '--no-sound', '--on-the-fly-encoding',
                                '--no-cursor', '--fps=1', '-o', addr],
                               stdout=subprocess.PIPE,
                               universal_newlines=True)
    recording_obj = {'proc': process, 'addr': addr + '.ogv'}
    return recording_obj

def stop_record(recording_obj, keep=True):
    process = recording_obj['proc']
    process.send_signal(signal.SIGINT)   # send Ctrl-C signal
    stdout, stderr = process.communicate()
    if keep is not True:
        os.remove(recording_obj['addr'])
...
def handle_config_file(fv):
    ...
    print(cfg.base_directory+"e2e.conf", flush=True)
...
def copy_to_host(fv, suffix=None):
    if suffix is None:
        suffix = ''
...
def run_single_scenario(fv_arg, suffix=None):
    ...
    # copy_to_host(fv, suffix=suffix)
    file_name = 'Results/' + str(fv) + suffix
    if os.path.exists(file_name):
        DfC_min, DfV_max, DfP_max, DfM_max, DT_max, traffic_lights_max = get_values(file_name)
...
def run(fv, suffix=None):
    ...
    try:
        ...
        counter = 0
        time.sleep(1)
        rec_obj = start_record('Results/' + str(fv).replace(' ', ''))
        while (True):
            counter = counter + 1
            time.sleep(1)
        ...
        stop_record(rec_obj, keep=False)
        copy_to_host(fv, suffix=suffix)
        stop_pylot_container()
