...
regex = re.compile(
    "\d{2}-\d{2}-\d{4} \d{2}:\d{2}:\d{2} \| Location\(x=[^,\n]+, y=[^,\n]+, z=[^,\n]+\)>DE:([^,\n]+),DfC:([^,\n]+),DfV:([^,\n]+),DfP:([^,\n]+),DfM:([^,\n]+),DT:([^,\n]+),DiSO:([^,\n]*),DiLS:([^,\n]*),DiRI:([^,\n]*),DiID:([^,\n]*)"
)
...
if __name__ == "__main__":
    ...
    time_df["DE"] = (
        time_df.set_index("run")["DE"] - time_df.groupby("run").first()["DE"]
    ).to_list()
    time_ord_df = (
        time_df.drop("DE", axis=1)
        .iloc[:, : columns.index("DT")]
        .unstack()
        .to_frame()
        .reset_index(0)
        .join(time_df[["DE", "run"]])
        .rename(columns={"level_0": "col", 0: "val"})
    ...
    for col in columns[1 : columns.index("DT") + 1]:
        ...
        if LINE_PLOT is True:
            sns.lineplot(x="DE", y="val", ci=None, hue="run", data=time_hist)
        # else:
