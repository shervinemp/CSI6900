...
def event6(input_df):
    if input_df.DiLS.hasnans:
        return 0
    edge = (
        input_df.DiLS.map(lambda x: x.split("-")[1].split("_")[1])
        .map({"Red": 0, "Yellow": 1, "Green": 2})
        .diff()
        .fillna(0)
        != 0
    )
    ...
    if input_df.DiLS.hasnans:
        return 0
    light_change = list(
        map(list, input_df[edge & ~ends].groupby("run").groups.values())
    )
    max_len = max(map(len, light_change))
    padded = [
        (x + [m for i in range(max_len - len(x))])
        for x, m in zip(light_change, run_len)
    ]
...
def hard_flaky(whole_df):
    ...
    return (
        hard_df.groupby(val_cols)[fit_cols]
        .agg(lambda x: entropy([x.sum(), x.count() - x.sum()], base=2))
        .astype(float)
    )
