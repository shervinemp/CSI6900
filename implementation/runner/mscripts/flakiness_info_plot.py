...
if __name__ == "__main__":
    ...
    t = list(zip((r := [-1, 0.0, 0.01, 0.05, 0.1, 0.2, 0.4, 0.7, 1]), r[1:]))
    delta_max = delta.max()
    delta_flaky = {
        (x1, x2): pd.concat(
            [
                (d := ((delta > x1 * delta_max) & (delta <= x2 * delta_max)).sum()),
                d / len(data),
            ],
            axis=1,
    ...
    t2 = np.linspace(-0.1, 0.1, 3, endpoint=True)

    df_range_delta = df.max() - df.min()
    mean_thresh = {
        x: pd.concat(
            [(d := (mean <= x * df_range_delta).sum()), d / len(data)], axis=1
        ).rename(columns={0: "count", 1: "percent"})
        for x in t2
    }
    ...
    mi_thresh = {
        x: pd.concat(
            [(d := (mi <= x * df_range_delta).sum()), d / len(data)], axis=1
        ).rename(columns={0: "count", 1: "percent"})
        for x in t2
    }
    mm_thresh = {
        x: pd.concat(
            [
                (d := ((mi <= x * df_range_delta) & (ma > x * df_range_delta)).sum()),
                d / len(data),
            ],
            axis=1,
        ).rename(columns={0: "count", 1: "percent"})
    ...
    data_ = [
        [((diff_[f] > a) & (diff_[f] <= b)).sum() for a, b in zip(bins, bins[1:])]
        for f in fit_cols
    ]
    ...
    mean_melt = mean.melt(
        value_vars=fit_cols,
        var_name="fit_id",
        value_name="mean_vals",
        ignore_index=False,
    ).set_index(["fit_id"], append=True)
    diff_melt = diff.melt(
        value_vars=fit_cols,
        var_name="fit_id",
        value_name="delta_vals",
        ignore_index=False,
    ).set_index(["fit_id"], append=True)
    mean_diff = (
        pd.merge(mean_melt, diff_melt, left_index=True, right_index=True)
        .reset_index()
        .set_index(diff.index.names)
    )

    ax = sns.lineplot(
        data=mean_diff.reset_index(drop=True),
        x="mean_vals",
        y="delta_vals",
        hue="fit_id",
    )
    ...
    metrics = pd.concat(
        [delta, var], axis=1, keys=["delta", "var"], names=["metric", "fitness"]
    )
    ...
    get_bins = lambda ma: [
        x
        for i, x in enumerate(np.linspace(0, ma, 21, endpoint=True))
    ...
    for i, m in enumerate(["delta", "var"]):
        for j, col in enumerate(fit_cols):
            ...
            sns.histplot(
                x=col,
                data=metrics[m],
                bins=get_bins(metric_max),
                binrange=(0, metric_max),
                ax=ax,
            )
            ...
            ax.set_xticklabels(
                ["{:.0f}%".format(x) for x in np.linspace(0, 100, 6, endpoint=True)]
            )
    # Tightly adjust the layout of the plots
    fig.tight_layout(rect=[0, 0.03, 1, 0.95])
    # Save the plot to a file
    ...
    pprint({f: wilcoxon((df_list[-1][f] - df_list[0][f]).to_list()) for f in fit_cols})
